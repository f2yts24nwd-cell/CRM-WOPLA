...
const stimmungKlasse = computed(() => {
  if (!kiErgebnis.value) return ''
  if (kiErgebnis.value.stimmung === 'positiv') return 'text-positive bg-positive/10'
  if (kiErgebnis.value.stimmung === 'kritisch') return 'text-negative bg-negative/10'
      ...
      <div class="bg-surface rounded-t-xl flex flex-col max-h-[90dvh]">
        ...
        <div class="flex-shrink-0 pt-3 pb-2 px-4 border-b border-border">
          ...
          <div class="flex items-center justify-between">
            <div>
              <h2 class="font-bold text-text1">Besuchsbericht</h2>
              <p class="text-xs text-text2 mt-0.5">{{ kunde?.name }}</p>
            </div>
        ...
        <div class="flex-1 overflow-y-auto px-4 py-4">
          ...
          <div v-if="schritt === 'aufnahme'" class="space-y-4">
            <div class="flex flex-col items-center py-6">
              <button
                ...
                :class="[
                  ...
                  isRecording
                    ? 'bg-negative scale-110 animate-pulse'
                    : 'bg-brand-active hover:bg-brand active:scale-95'
                ]"
              >
              ...
              <p class="mt-3 text-sm font-semibold text-text1">
            ...
            <div>
              <label class="text-xs font-semibold text-text2 mb-1.5 block">Bericht (Text)</label>
              <textarea
                v-model="transkript"
                placeholder="Alternativ hier eintippen…"
                rows="4"
                class="w-full rounded border border-border bg-input px-3 py-2.5 text-sm text-text1 resize-none focus:outline-none focus:ring-2 focus:ring-brand/50"
              />
            </div>
            ...
            <button
              ...
              class="w-full py-3 rounded font-semibold text-white bg-brand-active active:bg-brand disabled:opacity-40"
            >
          ...
          <div v-else-if="schritt === 'verarbeitung'" class="flex flex-col items-center py-16 gap-4">
            ...
            <p class="text-sm font-semibold text-text1">KI analysiert Bericht…</p>
            <p class="text-xs text-text2 text-center">Zusammenfassung und Folgeaktionen werden erstellt</p>
          </div>

          <!-- Schritt 3: Ergebnis -->
          <div v-else-if="schritt === 'ergebnis' && kiErgebnis" class="space-y-4">
            <div :class="['rounded-lg p-4', stimmungKlasse]">
            ...
            <div>
              <p class="text-sm font-bold text-text1 mb-2">Vorgeschlagene Folgeaktionen</p>
              <div class="space-y-2">
                <label
                  v-for="(aktion, i) in kiErgebnis.folgeaktionen"
                  :key="i"
                  class="flex items-start gap-3 bg-surface2 rounded-lg p-3 cursor-pointer active:bg-border/30"
                >
                  <input
                    type="checkbox"
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-1.5 mb-0.5">
                      ...
                      <span class="ml-auto text-xs text-brand font-medium shrink-0">
                        {{ formatFaellig(aktion.faelligAm) }}
                      </span>
                    </div>
                    <p class="text-sm text-text1">{{ aktion.beschreibung }}</p>
                  </div>
                </label>
              </div>
            </div>
        ...
        <!-- Footer-Button -->
        <div v-if="schritt === 'ergebnis'" class="flex-shrink-0 px-4 py-4 border-t border-border"
          style="padding-bottom: calc(env(safe-area-inset-bottom) + 1rem)">
          <button
            @click="speichernUndFertig"
            class="w-full py-3 rounded font-semibold text-white bg-positive active:bg-positive/80"
          >
            Speichern & Fertig
          </button>
        </div>
