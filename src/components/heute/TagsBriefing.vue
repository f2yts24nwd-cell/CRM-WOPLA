...
<template>
  <div class="mx-3 mt-3 rounded-lg border-l-4 border-brand bg-brand/5 overflow-hidden">
    <button
    ...
    >
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-semibold text-brand">KI-Tages-Briefing</span>
        <LoadingSpinner v-if="ki.isLoading && ki.currentTask === 'briefing'" size="sm" />
      </div>
      <svg :class="['w-4 h-4 text-text2 transition-transform', collapsed ? '' : 'rotate-180']"
    ...
    <div v-if="!collapsed && briefing" class="px-4 pb-3 space-y-2">
      <div class="flex items-start gap-2">
        <span class="text-xs font-semibold text-brand shrink-0 mt-0.5">Priorität</span>
        <span class="text-xs text-text1">{{ briefing.prioritaet }}</span>
      </div>
      <div class="flex items-start gap-2">
        <span class="text-xs font-semibold text-positive shrink-0 mt-0.5">Route</span>
        <span class="text-xs text-text1">{{ briefing.routeStatus }}</span>
      </div>
      <div class="flex items-start gap-2">
        <span class="text-xs font-semibold text-text2 shrink-0 mt-0.5">Tipp</span>
        <span class="text-xs text-text1 italic">{{ briefing.hinweis }}</span>
      </div>
    </div>

    <div v-else-if="!collapsed && !briefing && !(ki.isLoading && ki.currentTask === 'briefing')"
      class="px-4 pb-3 text-xs text-text2">
