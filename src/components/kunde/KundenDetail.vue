...
type Tab = 'uebersicht' | 'ansprechpartner' | 'aktivitaeten' | 'opportunities' | 'angebote'
const activeTab = ref<Tab>('uebersicht')

const kundenStore = useKundenStore()
const ki = useKiStore()

const kunde = computed(() => kundenStore.getKundeById.value(props.kundeId))
const ansprechpartner = computed(() => kundenStore.getAnsprechpartnerForKunde.value(props.kundeId))
const aktivitaeten = computed(() => kundenStore.getAktivitaetenForKunde.value(props.kundeId))
const opportunities = computed(() => kundenStore.getOpportunitiesForKunde.value(props.kundeId))
const angebote = computed(() => kundenStore.getAngeboteForKunde.value(props.kundeId))
...
const tabs: { key: Tab; label: string }[] = [
  ...
  { key: 'ansprechpartner', label: 'Kontakte' },
...
<template>
  <div v-if="kunde" class="fixed inset-0 z-40 bg-white flex flex-col">
    ...
    <div class="bg-white border-b border-gray-200 px-4 pt-4 pb-3"
      ...
      <div class="flex gap-1 overflow-x-auto no-scrollbar -mx-1 px-1">
        <button
          ...
          :class="[
            'shrink-0 px-3 py-2 rounded-lg text-xs font-semibold transition-colors whitespace-nowrap',
            activeTab === tab.key
    ...
    <div class="flex-1 overflow-y-auto">
      <!-- Übersicht -->
      <div v-if="activeTab === 'uebersicht'" class="p-4 space-y-4">
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="bg-gray-50 rounded-xl p-3">
            ...
            <p class="text-xs text-teal-500">{{ kunde.branche }}</p>
          </div>
        </div>
        <div class="bg-gray-50 rounded-xl p-3">
        ...
        <div class="bg-gray-50 rounded-xl p-3">
          <p class="text-xs text-gray-400 mb-1">Offene Opportunities</p>
          <p class="font-semibold text-gray-900 text-sm">
            {{ opportunities.filter(o => o.status === 'offen').length }} offen
            · {{ angebote.filter(a => a.status === 'Gesendet').length }} Angebote gesendet
          </p>
        </div>
      </div>

      <!-- Ansprechpartner -->
      <div v-else-if="activeTab === 'ansprechpartner'" class="p-4 space-y-3">
        <div v-for="ap in ansprechpartner" :key="ap.id"
          class="bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
          <div class="flex items-start justify-between">
            <div>
              <p class="font-semibold text-gray-900 text-sm">{{ ap.name }}</p>
              <p class="text-xs text-teal-600 font-medium mt-0.5">{{ ap.rolle }}</p>
            </div>
            <div class="flex gap-2">
              <a :href="`tel:${ap.telefon}`"
                class="w-9 h-9 bg-primary-100 rounded-lg flex items-center justify-center">
          ...
          <p class="text-xs text-gray-400 mt-2">{{ ap.telefon }}</p>
          <p class="text-xs text-gray-400">{{ ap.email }}</p>
        </div>
      </div>

      <!-- Aktivitäten -->
      <div v-else-if="activeTab === 'aktivitaeten'" class="p-4">
        <div class="space-y-3">
      ...
      <!-- Opportunities -->
      <div v-else-if="activeTab === 'opportunities'" class="p-4 space-y-3">
        ...
        <div v-if="opportunities.length === 0" class="text-center py-8 text-gray-400 text-sm">
          Keine Opportunities
        </div>
      </div>

      <!-- Angebote -->
      <div v-else-if="activeTab === 'angebote'" class="p-4 space-y-3">
        ...
        <div v-if="angebote.length === 0" class="text-center py-8 text-gray-400 text-sm">
          Keine Angebote
        </div>
      </div>
    </div>

    <!-- KI Gesprächsvorbereitung Button -->
